...
print_logo() {
    ...
    local slogan="Craft Better Code"
    local content="${app_name} ${separator} ${slogan}"
    local border_color="${DIM}"
    
    local plain_text="Coraft · Craft Better Code"
...
for i in 1 2 3; do
    if curl -L -f --connect-timeout 10 --max-time 300 --progress-bar -o "$FILENAME" "$DOWNLOAD_URL" 2>&1 | sed -u 's/\r/\r     /g' >&2; then
